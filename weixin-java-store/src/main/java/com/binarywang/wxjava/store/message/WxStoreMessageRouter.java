...
public class WxStoreMessageRouter {
   ...
   */
  public WxStoreMessageRouter(ExecutorService executorService) {
   ...
   */
  public Object route(final WxStoreMessage message, final String content, final String appId,
    final Map<String, Object> context, final WxStoreService service, final WxSessionManager sessionManager) {
    // 如果是重复消息，那么就不做处理
    ...
    final List<WxStoreMessageRouterRule<? extends WxStoreMessage>> matchRules = new ArrayList<>();

    // 收集匹配的规则
    for (final WxStoreMessageRouterRule<? extends WxStoreMessage> rule : this.rules) {
    ...
    Object result = null;
    for (final WxStoreMessageRouterRule<? extends WxStoreMessage> rule : matchRules) {
      // 返回最后一个非异步的rule的执行结果
      if (rule.isAsync()) {
        futures.add(
   ...
   * @return 是否重复
   */
  protected boolean isMsgDuplicated(WxStoreMessage wxMessage) {
   ...
   * @return 消息id
   */
  protected String generateMessageId(WxStoreMessage wxMessage) {
   ...
   */
  private void sessionEndAccess(WxSessionManager sessionManager, WxStoreMessage message, boolean async) {
