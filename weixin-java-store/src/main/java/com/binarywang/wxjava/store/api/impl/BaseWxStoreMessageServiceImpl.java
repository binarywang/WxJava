...
import com.binarywang.wxjava.store.api.BaseWxStoreMessageService;
...
import com.binarywang.wxjava.store.bean.message.sharer.SharerChangeMessage;
 ...
 */
@Slf4j
public abstract class BaseWxStoreMessageServiceImpl implements BaseWxStoreMessageService {

  /** 消息路由器 */
  protected WxStoreMessageRouter router;

  public BaseWxStoreMessageServiceImpl(WxStoreMessageRouter router) {
    this.router = router;
    this.addDefaultRule();
  }
   ...
   * @param <T> 消息类型
   */
  protected <T extends WxStoreMessage> void addRule(Class<T> clazz, String event, Boolean async,
                                                      HandlerConsumer<T, String, String, Map<String, Object>, WxSessionManager> consumer) {
    WxStoreMessageRouterRule<T> rule = new WxStoreMessageRouterRule<>();
  ...
  protected <T extends WxStoreMessage> void addRule(Class<T> clazz, String event,
    HandlerConsumer<T, String, String, Map<String, Object>, WxSessionManager> consumer) {
    this.addRule(clazz, event, true, consumer);
  }

  @Override
  public void addRule(WxStoreMessageRouterRule<? extends WxStoreMessage> rule) {
    router.getRules().add(rule);
  }

  @Override
  public Object route(WxStoreMessage message, String content, String appId, final WxStoreService service) {
